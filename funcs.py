...
def load_dataset(dataset_name, n_samples):
    ...
    if dataset_name == 'MNIST':
        ...
        desired_labels = np.array([1, 3]) # Select a combination of classes of Fashion-MNIST data using the index of str_labels
        ind = []
    ...
    elif dataset_name == 'BNC-2007':

        data_read = pyreadr.read_r('bnc.rds') # load data from .rds file
...
def plot_clustering_results(root, most_informative_cluster, iteration, proj, patterns_ind, non_patterns_ind, predicted_labels, unexplored_data, non_patterns_labels, total_labels, colors):
    """
    ...
    for i, l in enumerate(np.unique(non_patterns_labels)):
        ...
        plt.tight_layout()
    plt.ioff()
    plt.savefig(root + 'Proj' + str(iteration) + '_Selected cluster_' + str(colors[most_informative_cluster]) + '.pdf')  # save the triplet
    return
...
def CalculateProjection(root, method, input_data, background_data, unexplored_data, alpha, mu, iteration, seed):
    ...
    x0 = np.random.normal(size=(input_data.shape[1], 2)) # Set initial point for the optimization
